...
class AlgoTrader():
    ...
    def long_real_entry(self, symbol, comment="NA"):
        ...
        if entry_price:
            _, stop_price, prev_can_dir = ind.get_stop_range(symbol, int(comment))
    ...
    def short_real_entry(self, symbol, comment="NA"):
        ...
        if entry_price:
            stop_price, _, previous_candle = ind.get_stop_range(symbol, int(comment))
                                ...
                                try:
                                    ...
                                    if signal and active_orders < 1:
                                        ...
                                        if signal == "L":
                                            ...
                                            if self.short_real_entry(symbol=symbol, comment=str(trading_time)):
                                                # Make sure we make only 1 trade at a time
                                                break
                                except Exception as e:
