...
class MyWindow(QMainWindow):
    ...
    def random_entry(self):
        import random
        self.direction = random.choice(["long", "short"])
    ...
    def trade_confirmation(self, points_in_stop, position_size, target_price1):
        input_string = f"{self.direction} - {self.symbol} with ${self.risk}<br> Dollar Value : ${self.dollor_value} <br>Risk ({points_in_stop})pips <br>Positions {position_size}<br> Target @ {target_price1}"
