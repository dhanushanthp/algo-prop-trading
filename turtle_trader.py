    ...
    def main(self):
        while True:
            ...
            for cancel_orders in possible_cancelling_orders:
                self.orders.cancel_single_pending_order(active_order=cancel_orders)

            # If current positions are upto daily limit then cancel all the pending orders
            ...
            if is_market_open and (not is_market_close) and self.wrapper.today_unique_traded_symbols():
                ...
                for symbol in curr.get_major_symbols(security=self.security):
                    ...
                    if (symbol in existing_positions) or (symbol in active_orders):
                        continue
